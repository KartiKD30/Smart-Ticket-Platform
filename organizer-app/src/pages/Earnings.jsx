...
import { listVariants, cardVariants, springTransition } from '../utils/motion';

/* ─── Skeleton for Earnings ─────────────────────────────────────────────── */
...
const Earnings = () => {
  ...
  /* ── Skeleton instead of blank screen ─────────────────────────── */
  if (loading) return <EarningsSkeleton />;

  const statItems = [
    {
      label: 'Total Earnings',
      value: payoutData?.totalEarnings?.toLocaleString(),
      icon: Wallet,
      tone: 'text-primary',
    ...
    },
    {
      ...
      value: payoutData?.pendingPayouts?.toLocaleString(),
      icon: Clock,
    ...
    },
    {
      ...
      value: payoutData?.completedPayouts?.toLocaleString(),
      icon: CheckCircle2,
      tone: 'text-green-500',
    ...
    <AnimatedPage>
      ...
      <AnimatedSection>
        ...
        <p className="mt-1 text-muted-foreground">
          Manage your revenue and track payout statuses.
        </p>
      </AnimatedSection>
      ...
      >
        {statItems.map((item) => (
          <motion.div
            key={item.label}
            className="panel-card flex flex-col justify-center"
            variants={cardVariants}
            whileHover={{
              y: -6,
              boxShadow: `0 28px 60px ${item.glow}, 0 10px 26px rgba(0,0,0,0.38)`,
          ...
          <AnimatedButton
            onClick={async () => {
              ...
              try {
                ...
                if (payoutData?.defaultEventId) payload.eventId = payoutData.defaultEventId;
            ...
            className="btn-glow rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90"
          >
            Request Payout
          </AnimatedButton>
        </div>
            ...
            {payoutData?.payoutHistory?.map((payout) => (
              ...
              >
                ...
                <td className="px-6 py-4 flex items-center gap-4">
                  <span
                    className={`badge-pulse flex w-fit items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium ${
                  ...
                  {payout.status === 'pending' && (
                    <motion.button
                      onClick={async () => {
                        try {
                          await api.put(`/payouts/${payout._id}/complete`);
                          fetchPayouts();
                        } catch (err) {
                          console.error(err);
                        }
