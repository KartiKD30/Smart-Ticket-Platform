...
const Toast = ({ msg, type, onDone }) => {
  ...
  return (
    <motion.div
      className={`fixed bottom-6 right-6 z-[9999] flex items-center gap-3 rounded-xl px-5 py-3 text-sm font-medium text-white shadow-xl ${bg}`}
...
const EventManagement = () => {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      setEvents([]);
      setLoadError(err.response?.data?.error || 'Could not load your events right now.');
      showToast('Could not load events right now. Please retry.', 'error');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <AnimatedPage className="space-y-6">
      ...
      {loading ? (
        ...
        <AnimatedSection className="panel-card p-12 text-center text-muted-foreground">
          <ImageIcon className="w-12 h-12 mx-auto mb-3 opacity-20" />
          <p className="font-medium">No events yet</p>
          <p className="text-sm mt-1">Click <strong>Create Event</strong> to get started.</p>
        </AnimatedSection>
      ) : (
        <motion.div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3" variants={listVariants} initial="initial" animate="animate">
          {events.map((event) => {
