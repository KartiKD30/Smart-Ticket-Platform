...
export default function Payment() {
  ...
  const [organizerPromos, setOrganizerPromos] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchOrganizerPromos = async () => {
      try {
        ...
        console.error("Failed to fetch organizer promos:", error);
        setOrganizerPromos([]);
      }
    };
  ...
  const walletEligibleAmount = Math.max(0, baseAmount - couponDiscount);

  // Calculate wallet deduction
  const maxWalletDeduction = useWallet ? Math.min(walletAmount, walletBalance, walletEligibleAmount) : 0;
  
  // Final amount after coupon discount and wallet
  const finalAmount = Math.max(0, baseAmount - couponDiscount - maxWalletDeduction);
  const walletRewardPreview = couponDiscount;
  ...
  useEffect(() => {
    ...
    setWalletAmount((current) => Math.min(current, walletBalance, walletEligibleAmount));
  }, [useWallet, walletBalance, walletEligibleAmount]);

  const handleWalletToggle = (e) => {
    setUseWallet(e.target.checked);
    if (e.target.checked) {
      setWalletAmount(Math.min(walletBalance, walletEligibleAmount));
    } else {
      setWalletAmount(0);
    }
  };
  ...
  const applyCoupon = () => {
    ...
    const organizerPromo = organizerPromos.find((promo) => promo.code?.toUpperCase() === normalizedCode);
    const offer = organizerPromo
      ? {
          id: organizerPromo._id,
          title: organizerPromo.eventId?.title || organizerPromo.eventId?.name || "Organizer Promo",
          description: "Promo code created by the organizer",
          discount: organizerPromo.discountType === "percentage"
            ? `${organizerPromo.discountValue}% OFF`
            : `Rs. ${organizerPromo.discountValue} OFF`,
          discountValue: organizerPromo.discountValue,
          discountType: organizerPromo.discountType === "fixed" ? "flat" : organizerPromo.discountType,
          category: "Organizer Promo",
          icon: "🏷️",
          code: organizerPromo.code,
          validTill: organizerPromo.expiryDate,
        }
      : getOfferByCode(normalizedCode);
  ...
  const availableCoupons = [
    ...organizerPromos.map((promo) => ({
      id: promo._id,
      title: promo.eventId?.title || promo.eventId?.name || "Organizer Promo",
      description: "Promo code created for this event",
      discount: promo.discountType === "percentage"
        ? `${promo.discountValue}% OFF`
        : `Rs. ${promo.discountValue} OFF`,
      discountValue: promo.discountValue,
      discountType: promo.discountType === "fixed" ? "flat" : promo.discountType,
      category: "Organizer Promo",
      icon: "🏷️",
      code: promo.code,
      validTill: promo.expiryDate,
    })),
    ...offersData,
  ];
    ...
    try {
      ...
      navigate("/success", {
        state: {
          ...
          discountDescription: couponDescription || `Coupon: ${appliedCoupon?.code || ''}`,
          walletDeducted: maxWalletDeduction,
          walletRewardCredit: response.data.walletRewardCredit ?? walletRewardPreview,
        },
      });
    } catch (error) {
              ...
              {useWallet && (
                <div className="payment-field">
                  ...
                  <small className="payment-hint">
                    Maximum: Rs. {Math.min(walletBalance, walletEligibleAmount).toLocaleString("en-IN")}
                  </small>
                </div>
              )}
            </div>
            ...
            {showAvailableOffers && (
              <div className="payment-offers-list">
                {availableCoupons.map((offer) => (
                  <button
                    key={`${offer.code}-${offer.id || "static"}`}
                    type="button"
